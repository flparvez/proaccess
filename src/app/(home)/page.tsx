...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const initData = async () => {
      try {
        const [catRes, prodRes] = await Promise.all([
          fetch("/api/categories", { cache: "force-cache", next: { revalidate: 60 } }),
  ...
  // ⚡ Variants Handlers
  const addVariant = () => {
    setVariants([...variants, { name: "", validity: "", price: 0 }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        variants: variants.filter(v => v.name && v.validity), // Filter empty variants
          ...
          <Card className="border-blue-100 bg-blue-50/20 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center justify-between text-lg text-blue-900">
                <div className="flex items-center gap-2"><Layers className="w-5 h-5"/> Product Variants</div>
                <Button size="sm" variant="secondary" onClick={addVariant} className="bg-blue-100 text-blue-700 hover:bg-blue-200">
                   <Plus className="w-4 h-4 mr-1"/> Add Variant
                </Button>
              </CardTitle>
              <CardDescription>Create different packages (e.g. Silver, Gold) with specific validities.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {variants.length === 0 && <p className="text-sm text-muted-foreground italic">No variants added yet. Uses default pricing.</p>}
              
              {variants.map((variant, index) => (
                <div key={index} className="flex flex-col md:flex-row gap-3 items-end border p-3 rounded-lg bg-white shadow-sm">
                  ...
                  <div className="space-y-1 flex-1 w-full">
                    <Label className="text-xs">Validity (Duration)</Label>
                    <div className="relative">
                      <Clock className="w-3 h-3 absolute left-2.5 top-3 text-muted-foreground" />
                      <Input 
                  ...
                  <Button variant="ghost" size="icon" onClick={() => removeVariant(index)} className="text-red-500 hover:bg-red-50 hover:text-red-600 mb-0.5">
